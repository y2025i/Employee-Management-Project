...
class EmployeeAppUI:
    ...
    def build_ui(self) -> None:
        ...
        with ui.column().classes('w-full gap-4'):
            ...
            with ui.row().classes('w-full items-start justify-center no-wrap').style('padding: 12px; gap: 16px;'):
                ...
                with ui.card().classes('shadow-2 w-[350px]').style('background-color: #EAF2F2; min-height: 560px;'):
                    ...
                    self.department = ui.select(
                        list(self.dept_positions.keys()), 
                        label='Department',
                        on_change=self.update_positions
                    ).classes('w-full mb-2')
                    
                    self.position = ui.select([], label='Position').classes('w-full mb-2')
                ...
                with ui.card().classes('shadow-2 flex-1').style('background-color: #EAF2F2; min-height: 560px; max-width: 800px;'):
                    ...
                    with ui.row().classes('w-full bg-white p-3 rounded items-center gap-3 mb-4'):
                        ...
                        ui.button('Show All', on_click=self.refresh_ui).props('color=grey')
